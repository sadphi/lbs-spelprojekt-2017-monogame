...
namespace inkArenaGame
{
    class Player
    {
        ...
        public Player(PlayerIndex playerIndex, int nx, Keys leftKey, Keys rightKey, Keys upKey, Keys downKey, Keys dabKey, Keys fireKey, Keys jumpKey)
        {
            index = playerIndex;
        ...
        public void Update(ref bool hit)
        {
            ...
            #region Arm Angle
            rightThumb = (Math.Abs(DirX()) + Math.Abs(DirY()) != 0 ? new Vector2(DirX(), -DirY()) : rightThumb);
        ...
        public void DabState()
        {
            ...
            if (newstate.IsKeyDown(dabKey))
            {
                state = State.dabing;
                if (oldstate.IsKeyUp(dabKey)) flipFrame = rnd.Next(0, 4) == 0 ? (flipFrame = !flipFrame) : flipFrame;
            }
            else
            {
                state = State.standing;
        ...
        private int DirX()
        {
            if (newstate.IsKeyDown(rightKey)) return 1;
            if (newstate.IsKeyDown(leftKey)) return -1;
            return 0;
        }

        private int DirY()
        {
            if (newstate.IsKeyDown(downKey)) return 1;
            if (newstate.IsKeyDown(upKey)) return -1;
            return 0;
        }

        private bool ButtonHit(Keys button)
        {
            return newstate.IsKeyDown(button) && oldstate.IsKeyUp(button);
        }
